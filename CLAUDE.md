...
`contents/ohenro-*/`（テンプレートは `ohenro-01/`）の日記本文を書く際は，お接待をしてくださった方・宿泊施設・他の遍路者など，第三者に迷惑や実害が及ばないよう次の点に留意する．四国遍路のお接待やブログ執筆について，これを包括的に定めた公式ガイドラインは調査した限り見当たらなかったため，お接待の文化的背景・一般的な個人情報保護の考え方・既存の遍路ブログで実際に起きているトラブル傾向（民宿を実名で酷評する記事など）を踏まえて，このプロジェクト独自の方針としてまとめたもの．
