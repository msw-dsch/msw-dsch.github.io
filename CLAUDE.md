...
- パート声部のラベルは文字（S/A/Tなど）ではなく，五線の左に描く音部記号（Google Fonts「Noto Music」の実際のグリフ，ト音記号＝U+1D11E／ヘ音記号＝U+1D122）で暗示する．スコア譜では声部ごとに1つ，パート譜ではその記事の声部の記号を1つだけ表示する．
...
- **カテゴリラベルの表示を撤廃**：記事ページの見出し上のラベル（`eyebrow`）が既定で「作品 ｜ 2021」のように `kind`（内部分類）を名乗っていたのをやめ，年のみ（「2021」）に変更．スコア譜のツールチップも「S ｜ 作品 ｜ 2021-03-09」から `kind` を外し「S ｜ 2021-03-09（デモ）」のように，声部・日付・デモ表記のみにした．`kind` は `source.html` の中では作者向けの分類として引き続き必須項目だが，サイト上には表示しない．composition/index.html・music/index.html などカテゴリ別の一覧ページ自体はカテゴリ表示を前提とした別物として変更していない．
